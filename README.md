...
- The conformance suite still has failures, primarily in cycle/dot-accurate PPU, DMA, interrupt, and hardware-revision behavior. All SameSuite APU cases assigned to the selected DMG-B and CGB-E targets pass, as does the single revision-specific AGB-A execution currently assigned by the manifest; six fixtures for other revisions remain explicit skips. `mooneye/acceptance/halt_ime0_nointr_timing` is currently deferred: resolving its remaining one-cycle discrepancy requires coordinated HALT wake, interrupt-polling, and VBlank phase modeling rather than another local timing adjustment. It remains a visible failing test instead of being suppressed. The core remains experimental while these failures remain.
