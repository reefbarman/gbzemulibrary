...
`Emulator.Config(HardwareModel)` requires a concrete model and accepts one immutable `BootRomConfig`: built-in firmware, an external file, external bytes, or skip. `Core/BootROM.cs` validates exact model-specific images: 256 bytes for DMG-B, MGB, and SGB2, and 2304 bytes for CGB-E and AGB-A. External byte-backed configuration and active firmware storage must remain privately owned. Built-in replacement firmware is maintained under `GBZEmuLibrary/BootROMs/`; official Nintendo firmware is never distributed.

Any boot change must cover DMG-only, CGB-compatible, and CGB-only cartridge headers across implemented DMG-B, MGB, CGB-E, AGB-A, and SGB2 models and all four firmware choices where applicable. Preserve the DMG-B/MGB/SGB2 overlay at `0x0000–0x00FF`, the additional CGB-E/AGB-A overlay at `0x0200–0x08FF`, the shared `$FF50` unmap transition, and fallback to cartridge ROM outside the selected image.
