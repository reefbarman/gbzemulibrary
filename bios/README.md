...
| `sgb_boot.asm` | 256 bytes  | Forwards the cartridge header through JOYP, displays its logo, and hands off with SGB register state                          |
| `sgb2_boot.asm` | 256 bytes | Uses the same open replacement with the SGB2 accumulator handoff value                                                        |
...
- The SGB images are adapted from SameBoy's Expat-licensed replacement firmware.
  GBZEmu's original SNES-side wordmark and decorative border are generated by the
  HLE SGB system; the 256-byte bootstrap remains focused on header transfer and
  correct model handoff.
